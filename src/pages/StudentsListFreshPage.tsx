...
/** Fresh = newly registered this session + mock rows admitted in current term (adminDate contains 2020) */
function getFreshRows(registered: StudentRow[], merged: StudentRow[]): StudentRow[] {
  const registeredIds = new Set(registered.map((r) => r.studentId))
  return merged.filter(
    (r) => registeredIds.has(r.studentId) || r.adminDate.includes('2020')
  )
}
...
export default function StudentsListFreshPage() {
  ...
  const allRows = useMemo(() => {
    ...
    return getFreshRows(registeredStudents, merged)
  }, [registeredStudents])
  ...
  const handleExportExcel = () => {
    exportToExcel(COLUMNS, exportRows, 'students-fresh-list', 'Students Fresh', columnVisibility)
    showFeedback('Excel file downloaded.')
  }
  const handleExportCSV = () => {
    exportToCSV(COLUMNS, exportRows, 'students-fresh-list.csv', columnVisibility)
    showFeedback('CSV file downloaded.')
  }
  const handleExportPDF = () => {
    exportToPDF(COLUMNS, exportRows, 'students-fresh-list.pdf', 'Students List (Fresh)', columnVisibility)
    ...
    <div className="students-list-all-page">
      <header className="students-list-all-header">
        <div className="students-list-all-heading">
          <h1 className="students-list-all-title">Manage Student - Fresh</h1>
          <nav className="students-list-all-breadcrumb" aria-label="Breadcrumb">
            <Link to="/admin">Home</Link>
            {' / '}
            <Link to="/admin/students/list/fresh">Student List</Link>
          </nav>
                        ...
                        {openActionStudentId === row.studentId && (
                          ...
                          >
                            {ACTION_MENU_ITEMS.map((item) => {
                              ...
                              const linkState = { from: 'students-fresh' as const }
                              if (item.id === 'profile') {
                                return (
                                  <Link
                                    key={item.id}
