...
const RecordSingle = () => {
  ...
  const loadBillsForStudent = useCallback(async (studentId, academicYear, term) => {
    try {
      ...
      if (academicYear) {
        filtered = filtered.filter(b => b.academicYear === academicYear);
      }
      if (term) {
        filtered = filtered.filter(b => b.term === term);
      }
      // Only show pending/partial bills
      filtered = filtered.filter(b => b.status === 'pending' || b.status === 'partial');
      setBills(filtered);
    } catch (error) {
