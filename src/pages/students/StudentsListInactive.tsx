  ...
  const handleExport = (format: 'copy' | 'excel' | 'csv' | 'pdf'): void => {
    ...
    switch (format) {
      case 'copy':
        ...
        break;
      case 'excel':
        exportToExcel(exportData, `students-inactive-${new Date().toISOString().split('T')[0]}.xlsx`, columns);
        toast.showSuccess('Data exported to Excel');
        break;
      case 'csv':
        exportToCSV(exportData, `students-inactive-${new Date().toISOString().split('T')[0]}.csv`, columns);
        toast.showSuccess('Data exported to CSV');
        break;
      case 'pdf':
        const printContent = document.getElementById('student-table');
        if (printContent) {
          exportToPDF(printContent, `students-inactive-${new Date().toISOString().split('T')[0]}.pdf`);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-3">
          <div>
            ...
            <div className="flex items-center gap-2 text-gray-600 text-xs sm:text-sm">
              ...
              <span>/</span>
              <span className="text-gray-900 font-medium">Inactive Students</span>
            </div>
          </div>
        </div>
      </div>
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-gray-500">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <div className="text-lg font-semibold">No inactive students found</div>
                    </div>
                  </td>
                </tr>
              ) : (
