...
const FreshStudents: React.FC = () => {
  ...
  const loadStudents = useCallback(async (): Promise<void> => {
    try {
      setLoading(true);
      const freshStudents = await studentsService.getFresh();
      setStudents(freshStudents);
    } catch (error) {
      console.error('Error loading fresh students:', error);
      toast.showError('Failed to load fresh students');
    } finally {
      setLoading(false);
    }
  }, [toast]);
  ...
  return (
    <Layout>
      <div className="mb-5 sm:mb-6 md:mb-8">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-2">Fresh Students</h1>
      ...
      <div className="bg-white rounded-lg p-4 sm:p-6 shadow-md border border-gray-200 mb-4">
        ...
        <div className="text-sm text-gray-600 mb-4">
          New Students (Last 30 days): {filteredStudents.length}
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
        {loading ? (
          <div className="p-8 text-center">
            ...
            <div className="text-gray-500">Loading fresh students...</div>
          </div>
        ) : filteredStudents.length === 0 ? (
          <div className="p-8 text-center text-gray-500">
            <i className="fas fa-inbox text-4xl mb-3 text-gray-300"></i>
            <div>No fresh students found</div>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead className="bg-primary-500">
                <tr>
                  ...
                  <th className="p-3 text-left text-white text-xs uppercase">Admission Date</th>
              ...
              <tbody>
                {filteredStudents.map((student) => (
                  <tr key={student.id} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-3 text-sm">{student.admissionDate || 'N/A'}</td>
                    <td className="p-3">
                      <button
