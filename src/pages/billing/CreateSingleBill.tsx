...
const CreateSingleBill: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    if (!formData.class || !formData.student || !formData.academicYear || !formData.term || 
        !formData.billDate || billItemRows.length === 0) {
    ...
    try {
      ...
      toast.showSuccess(`Bill created successfully! Total Amount: GHS ${grandTotal.toFixed(2)}`);
      handleClear();
    } catch (error: any) {
              ...
              <div>
                ...
                <div className="relative select-dropdown-wrapper">
                  <select
                    ...
                    className="select-dropdown w-full px-4 py-2.5 border-2 border-gray-200 rounded-md text-sm min-h-[44px] disabled:bg-gray-100 disabled:cursor-not-allowed"
                  >
                    <option value="">{formData.class ? 'Select Student' : 'Select class group first'}</option>
