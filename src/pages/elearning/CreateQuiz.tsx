  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      await elearningService.createQuiz({
        ...
        date: formData.date || '',
        duration: formData.duration ? parseInt(formData.duration) : 30,
        questions: formData.questions ? parseInt(formData.questions) : 10,
        passingScore: formData.passingScore ? parseInt(formData.passingScore) : 60
      });
    ...
    <Layout>
      <div className="mb-5 sm:mb-6 md:mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-3">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-2">Create Quiz</h1>
      ...
      <div className="bg-white rounded-lg p-4 sm:p-5 md:p-6 shadow-md border border-gray-200">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
            ...
            <div>
              <label className="block mb-2 font-semibold text-gray-900 text-sm">
                Quiz Date <span className="text-red-500">*</span>
              </label>
              <input
                type="date"
                name="date"
