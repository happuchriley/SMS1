...
const Courses = () => {
  const [courses, setCourses] = useState([
    ...
    { id: 4, code: 'ICT101', title: 'Information Technology', instructor: 'Mr. Mike Wilson', level: 'JHS 2', students: 20, status: 'active' },
  ]);
  ...
  const filteredCourses = useMemo(() => {
    return courses.filter(course => {
      ...
      const matchesLevel = filterLevel === 'all' || course.level.startsWith(filterLevel.split(' ')[0]);
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 mb-3">
                      <i className="fas fa-user-tie mr-1"></i>{course.instructor}
                    </p>
                  </div>
                </div>
                <div className="flex items-center justify-between text-sm text-gray-600 mb-4 pb-4 border-b border-gray-200">
                  <span>
                    <i className="fas fa-layer-group mr-1"></i>{course.level}
                  </span>
                  <span>
                    <i className="fas fa-users mr-1"></i>{course.students} students
                  </span>
                </div>
