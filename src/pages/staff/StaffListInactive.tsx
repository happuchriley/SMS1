...
const StaffListInactive: React.FC = () => {
  ...
  const loadStaff = useCallback(async (): Promise<void> => {
    try {
      setLoading(true);
      const inactiveStaff = await staffService.getInactive();
      setStaff(inactiveStaff);
    } catch (error) {
      console.error('Error loading inactive staff:', error);
      toast.showError('Failed to load inactive staff');
    } finally {
      setLoading(false);
    }
  }, [toast]);
  ...
  const handleExport = (format: 'copy' | 'excel' | 'csv' | 'pdf'): void => {
    ...
    switch (format) {
      case 'copy':
        ...
        break;
      case 'excel':
        exportToExcel(exportData, `staff-inactive-${new Date().toISOString().split('T')[0]}.xlsx`, columns);
        toast.showSuccess('Data exported to Excel');
        break;
      case 'csv':
        exportToCSV(exportData, `staff-inactive-${new Date().toISOString().split('T')[0]}.csv`, columns);
        toast.showSuccess('Data exported to CSV');
        break;
      case 'pdf':
        const printContent = document.getElementById('staff-table');
        if (printContent) {
          exportToPDF(printContent, `staff-inactive-${new Date().toISOString().split('T')[0]}.pdf`);
    ...
    <Layout>
      <div className="mb-5 sm:mb-6 md:mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-3">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-2">Inactive Staff</h1>
            <div className="flex items-center gap-2 text-gray-600 text-xs sm:text-sm">
              ...
              <span>/</span>
              <span className="text-gray-900 font-medium">Inactive Staff</span>
            </div>
          </div>
        </div>
      </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-gray-500">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <div className="text-lg font-semibold">No inactive staff found</div>
                    </div>
                  </td>
                </tr>
              ) : (
                paginatedStaff.map((s, index) => (
                  <tr key={s.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">{s.department || 'N/A'}</td>
