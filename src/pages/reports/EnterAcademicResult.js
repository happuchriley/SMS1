...
const EnterAcademicResult = () => {
  ...
  const subjects = ['English Language', 'Mathematics', 'Science', 'Social Studies', 'Religious Studies'];
  const examTypes = ['Mid-Term Exam', 'End of Term Exam', 'Continuous Assessment'];
  ...
  const handleSubmit = async (e) => {
    ...
    if (incompleteResults.length > 0) {
      if (!window.confirm(`Some students do not have scores entered. Do you want to proceed with ${results.length - incompleteResults.length} results?`)) {
        return;
      }
    }
