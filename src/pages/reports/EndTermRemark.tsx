...
const EndTermRemark: React.FC = () => {
  ...
  const handleRemarkChange = (studentId: string, value: string): void => {
  ...
  const handleInsertTemplate = (studentId: string, template: string): void => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    if (incompleteRemarks.length > 0) {
      showDeleteModal({
        ...
        message: `Some students do not have remarks. Do you want to proceed with ${remarks.length - incompleteRemarks.length} remarks?`,
        onConfirm: async () => {
