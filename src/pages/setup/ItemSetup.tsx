...
const ItemSetup: React.FC = () => {
  ...
  const [items, setItems] = useState<ItemSetupItem[]>([]);
  const [loading, setLoading] = useState<boolean>(true);

  const [showModal, setShowModal] = useState<boolean>(false);
  const [editingItem, setEditingItem] = useState<ItemSetupItem | null>(null);
  ...
  const categories: string[] = ['Academic', 'Extracurricular', 'Administrative', 'Facilities', 'Other'];
  ...
  const loadItems = useCallback(async (): Promise<void> => {
    try {
      setLoading(true);
      const allItems = await setupService.getAllItemSetup();
      setItems(allItems as ItemSetupItem[]);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      if (editingItem && editingItem.id) {
        await setupService.updateItemSetup(editingItem.id, formData);
        toast.showSuccess('Item updated successfully');
      } else {
        await setupService.createItemSetup(formData);
      ...
      toast.showError(error.message || 'Failed to save item');
    }
  };

  const handleEdit = (item: ItemSetupItem): void => {
    setEditingItem(item);
    setFormData({
      code: item.code || '',
      name: item.name || '',
  ...
  const handleDelete = (id: string): void => {
    showDeleteModal({
      ...
      itemName: items.find(i => i.id === id)?.name || 'this item',
      onConfirm: async () => {
        try {
          await setupService.deleteItemSetup(id);
          toast.showSuccess('Item deleted successfully');
          await loadItems();
        } catch (error: any) {
          console.error('Error deleting item:', error);
          toast.showError(error.message || 'Failed to delete item');
        }
      }
    });
  };
  ...
  const handleCloseModal = (): void => {
    setShowModal(false);
    setEditingItem(null);
    setFormData({ code: '', name: '', category: '', amount: '', description: '', active: true });
  };
    ...
    <Layout>
      <div className="mb-5 sm:mb-6 md:mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-3">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-2">Item Setup</h1>
            <div className="flex items-center gap-2 text-gray-600 text-xs sm:text-sm">
              ...
              <span>/</span>
              <span className="text-gray-900 font-medium">Item Setup</span>
            </div>
          </div>
          <button
