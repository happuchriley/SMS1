...
import { getParentsListRows, type ParentListRow } from '@/data/adminMock'
...
const COLUMNS: ExportColumn[] = [
  ...
  { id: 'resAddress', label: 'Res.Address' },
]

function matchParent(row: ParentListRow, query: string): boolean {
  ...
  return (
    ...
    row.resAddress.toLowerCase().includes(q)
  )
}

const PARENTS_ROWS = getParentsListRows()
...
export default function ParentsListPage() {
  ...
  const filteredRows = useMemo(() => {
    if (!searchQuery.trim()) return PARENTS_ROWS
    return PARENTS_ROWS.filter((row) => matchParent(row, searchQuery))
  }, [searchQuery])
  ...
  const handleExportExcel = () => {
    exportToExcel(COLUMNS, exportRows, 'parents-list', 'Parents List', columnVisibility)
    showFeedback('Excel file downloaded.')
  }
  const handleExportCSV = () => {
    exportToCSV(COLUMNS, exportRows, 'parents-list.csv', columnVisibility)
    showFeedback('CSV file downloaded.')
  }
  const handleExportPDF = () => {
    exportToPDF(COLUMNS, exportRows, 'parents-list.pdf', 'Parent/Guardian List', columnVisibility)
    ...
    <div className="students-list-all-page">
      <header className="students-list-all-header">
        <div className="students-list-all-heading">
          <h1 className="students-list-all-title">Parent/Guardian List</h1>
          <nav className="students-list-all-breadcrumb" aria-label="Breadcrumb">
            <Link to="/admin">Home</Link>
            {' / '}
            <Link to="/admin/students/parents">Parents List</Link>
          </nav>
              ...
              {paginatedRows.map((row, idx) => (
                ...
                >
                  ...
                  {columnVisibility.class && (
                    <td className="students-list-all-td">{row.class}</td>
                  )}
                  {columnVisibility.mother && (
                    <td className="students-list-all-td">{row.mother}</td>
                  )}
                  {columnVisibility.father && (
                    <td className="students-list-all-td">{row.father}</td>
                  )}
                  {columnVisibility.email && (
                    <td className="students-list-all-td">{row.email}</td>
                  )}
                  {columnVisibility.resAddress && (
                    <td className="students-list-all-td">{row.resAddress}</td>
                  )}
                </tr>
              ))}
            </tbody>
