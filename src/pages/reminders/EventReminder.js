...
Thank you.`;
  ...
  const handleChange = (e) => {
    ...
    if (name === 'eventTitle' || name === 'eventDate' || name === 'eventTime') {
      const updatedMessage = defaultMessage
        .replace('[Event Title]', formData.eventTitle || '[Event Title]')
        .replace('[Date]', formData.eventDate || '[Date]')
        .replace('[Time]', formData.eventTime || '[Time]');
      
      if (name !== 'message') {
        setFormData(prev => ({ ...prev, [name]: value, message: prev.message || updatedMessage }));
      }
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.recipientType === 'Specific Class' && !formData.class) {
