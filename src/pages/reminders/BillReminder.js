...
const BillReminder = () => {
  ...
  // Filter students by selected class and get balances
  const filteredStudents = useMemo(() => {
    ...
    return allStudents
      ...
      .map(s => ({
        ...
        email: s.email,
        balance: 0 // Will be loaded separately
      }));
...
This is a reminder that your ward has an outstanding bill balance. Please make payment at your earliest convenience.
...
Thank you.`;
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await remindersService.sendBillReminder({
      ...
      toast.showSuccess(`${result.sent} bill reminder(s) sent successfully via ${formData.reminderType.toUpperCase()}!`);
      handleClear();
    } catch (error) {
      console.error('Error sending bill reminder:', error);
      toast.showError(error.message || 'Failed to send bill reminders');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePreview = () => {
    alert('Preview would show how the message will appear to recipients.');
  };
    ...
    <Layout>
      <div className="mb-5 sm:mb-6 md:mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-3">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-2">Bill Reminder</h1>
            <div className="flex items-center gap-2 text-gray-600 text-xs sm:text-sm">
              ...
              <span>/</span>
              <span className="text-gray-900 font-medium">Bill Reminder</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg p-4 sm:p-5 md:p-6 shadow-md border border-gray-200 mb-4 sm:mb-5">
        <div className="mb-6">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">Send Bill Reminder</h2>
          <p className="text-sm text-gray-600">Send bill payment reminders to parents/guardians.</p>
        </div>
                  ...
                  {filteredStudents.map(student => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-500">{student.id} - Balance: GHS {student.balance}</div>
