...
const DocumentCategories: React.FC = () => {
  ...
  const loadCategories = useCallback(async (): Promise<void> => {
    try {
      ...
      const catsWithCount = await Promise.all(cats.map(async (cat) => {
        const docs = await documentsService.getDocumentsByCategory(cat.id);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      if (editingCategory) {
        await documentsService.updateCategory(editingCategory.id, formData);
  ...
  const handleEdit = (category: CategoryWithCount): void => {
  ...
  const handleDelete = (id: string): void => {
    showDeleteModal({
      ...
      onConfirm: async () => {
        try {
          await documentsService.deleteCategory(id);
  ...
  const handleCloseModal = (): void => {
    ...
    setFormData({ name: '', description: '', color: 'blue', active: true });
  };
    ...
    <Layout>
      <div className="mb-5 sm:mb-6 md:mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-3">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-2">Document Categories</h1>
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-gray-600 text-xs sm:text-sm">
              ...
              <span>/</span>
              <span className="text-gray-900 font-medium">Document Categories</span>
            </div>
          </div>
          <button
          ...
          <div key={category.id} className="bg-white rounded-lg shadow-md border border-gray-200 p-5 hover:shadow-lg transition-shadow">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                <i className={`fas fa-folder ${getColorClass(category.color)} p-3 rounded-lg text-xl`}></i>
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">{category.name}</h3>
                  <p className="text-sm text-gray-600">{category.documentCount} documents</p>
                </div>
              </div>
