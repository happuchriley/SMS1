...
type InactiveRow = StudentRow & { bill: string; payment: string; balance: string }
...
export default function StudentsListInactivePage() {
  ...
  const allRows = useMemo<InactiveRow[]>(() => {
    const merged = mergeRegisteredWithMock(registeredStudents, MOCK_ROWS)
    return merged
      .filter((r) => r.status !== 'Active')
      .map((r) => ({ ...r, bill: '0.00', payment: '0.00', balance: '0.00' }))
  }, [registeredStudents])
  ...
  const handleExportExcel = () => {
    exportToExcel(COLUMNS, exportRows, 'students-inactive-list', 'Students Inactive', columnVisibility)
    showFeedback('Excel file downloaded.')
  }
  const handleExportCSV = () => {
    exportToCSV(COLUMNS, exportRows, 'students-inactive-list.csv', columnVisibility)
    showFeedback('CSV file downloaded.')
  }
  const handleExportPDF = () => {
    exportToPDF(COLUMNS, exportRows, 'students-inactive-list.pdf', 'Students List (Inactive)', columnVisibility)
  ...
  const linkState = { from: 'students-inactive' as const }
    ...
    <div className="students-list-all-page">
      <header className="students-list-all-header">
        <div className="students-list-all-heading">
          <h1 className="students-list-all-title">Students List - Inactive</h1>
          <nav className="students-list-all-breadcrumb" aria-label="Breadcrumb">
            <Link to="/admin">Home</Link>
            {' / '}
            <Link to="/admin/students/list/inactive">Students List - Inactive</Link>
          </nav>
            ...
            <tbody>
              {paginatedRows.map((row, idx) => (
                ...
                >
                  ...
                  {columnVisibility.bill && (
                    <td className="students-list-all-td">{row.bill}</td>
                  )}
                  {columnVisibility.payment && (
                    <td className="students-list-all-td">{row.payment}</td>
                  )}
                  {columnVisibility.balance && (
                    <td className="students-list-all-td">{row.balance}</td>
                  )}
