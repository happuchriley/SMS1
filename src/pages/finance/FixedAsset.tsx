...
const FixedAsset: React.FC = () => {
  ...
  const categories: string[] = ['Vehicle', 'Equipment', 'Furniture', 'Building', 'Land', 'Other'];
  const statuses: string[] = ['Active', 'Disposed', 'Under Maintenance'];
  ...
  const calculateCurrentValue = (purchaseCost: string, purchaseDate: string, depreciationRate: string): number => {
    if (!purchaseCost || !purchaseDate || !depreciationRate) return 0;
    const cost = parseFloat(purchaseCost);
    const rate = parseFloat(depreciationRate) / 100;
    const years = (new Date().getTime() - new Date(purchaseDate).getTime()) / (365.25 * 24 * 60 * 60 * 1000);
