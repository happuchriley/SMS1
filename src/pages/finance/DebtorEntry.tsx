...
const DebtorEntry: React.FC = () => {
  ...
  const paymentMethods: string[] = ['Cash', 'Cheque', 'Bank Transfer', 'Mobile Money', 'Other'];
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    if (!formData.date || !formData.debtorName || !formData.description || (!formData.debitAmount && !formData.creditAmount)) {
    ...
    setLoading(true);
    try {
      const debitAmount = formData.debitAmount ? parseFloat(formData.debitAmount) : 0;
      const creditAmount = formData.creditAmount ? parseFloat(formData.creditAmount) : 0;
      const amount = debitAmount > 0 ? debitAmount : creditAmount;
      
      await financeService.createDebtorEntry({
        ...formData,
        amount,
        debitAmount,
        creditAmount
      });
      toast.showSuccess('Debtor entry recorded successfully!');
      handleClear();
    } catch (error: any) {
      console.error('Error recording debtor entry:', error);
      toast.showError(error.message || 'Failed to record debtor entry');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Layout>
      <div className="mb-5 sm:mb-6 md:mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-3">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-2">Debtor Entry</h1>
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-gray-600 text-xs sm:text-sm">
              ...
              <span>/</span>
              <span className="text-gray-900 font-medium">Debtor Entry</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg p-4 sm:p-5 md:p-6 shadow-md border border-gray-200">
        <div className="mb-6">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">Record Debtor Transaction</h2>
          <p className="text-sm text-gray-600">Enter debtor transaction details (debit or credit).</p>
        </div>
