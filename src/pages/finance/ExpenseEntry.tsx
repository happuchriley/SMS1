...
const ExpenseEntry: React.FC = () => {
  ...
  const staffMembers: string[] = ['Admin User', 'Finance Officer', 'Accountant'];
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    if (!formData.date || !formData.expenseCategory || !formData.description || !formData.amount || !formData.paidBy) {
    ...
    setLoading(true);
    try {
      await financeService.createExpenseEntry({
        ...formData,
        amount: parseFloat(formData.amount),
        account: formData.expenseCategory
      });
      toast.showSuccess('Expense entry recorded successfully!');
      handleClear();
    } catch (error: any) {
      console.error('Error recording expense entry:', error);
      toast.showError(error.message || 'Failed to record expense entry');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Layout>
      <div className="mb-5 sm:mb-6 md:mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 mb-3">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-2">Expense Entry</h1>
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-gray-600 text-xs sm:text-sm">
              ...
              <span>/</span>
              <span className="text-gray-900 font-medium">Expense Entry</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg p-4 sm:p-5 md:p-6 shadow-md border border-gray-200">
        <div className="mb-6">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">Record Expense Transaction</h2>
          <p className="text-sm text-gray-600">Enter expense transaction details.</p>
        </div>
